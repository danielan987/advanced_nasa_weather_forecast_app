    ...
    if data and parameter in data:
        ...
        with st.spinner("Analyzing and visualizing data 📊..."):
            ...
            if parameter != "PRECTOTCORR":
                ax.axhline(y=config["low_threshold"], color="black", linestyle="--") 
            ...
            if parameter != "PRECTOTCORR":
                ax2.axhline(y=config["low_threshold"], color="black", linestyle="--")
            st.title("🔮")
            ...
            if parameter != "PRECTOTCORR":
                ax3.axhline(y=config["low_threshold"], color="black", linestyle="--")
            st.title("📈📉")
            ax3.set_title("Trend")
            ax3.set_xlabel("Date")
        ...
        if st.button("📁 Prepare Data for Export"):
            with st.spinner("Finalizing Excel file for download 🗂️..."):
                ...
                forecast_export.columns = ["Date", f"Forecast ({config['unit']})", f"Lower Interval Bound ({config['unit']})", f"Upper Interval Bound ({config['unit']})"]
